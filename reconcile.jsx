...
const RC_STATES = {
  ...
  unmatched:  { label: 'Unmatched',  scope: 'user', desc: 'No matching deposit linked yet.' },
...
// Age-based escalation is INTENTIONAL and documented here as one state machine
// (feedback item 7). Every screen derives its label from this, so no screen
// invents its own vocabulary. Escalation is a function of age, not condition:
//   < 24h   watch   — likely to resolve itself on the next payout
//   1–3 d   held    — set aside for manual review
//   > 3 d   action  — requires an operator decision
const RC_ESCALATION = [
  { key: 'watch',  maxHours: 24,       next: 'held',   nextIn: 'Held' },
  { key: 'held',   maxHours: 72,       next: 'action', nextIn: 'Needs Action' },
  { key: 'action', maxHours: Infinity, next: null,     nextIn: null }
];
const rcHours = (age) => {
  const n = parseFloat(age) || 0;
  return /d/i.test(age) ? n * 24 : n;
};
const rcEscalate = (age) => {
  const h = rcHours(age);
  const stage = RC_ESCALATION.find(s => h < s.maxHours) || RC_ESCALATION[RC_ESCALATION.length - 1];
  const hoursLeft = stage.maxHours === Infinity ? null : Math.max(0, Math.round(stage.maxHours - h));
  return { status: stage.key, next: stage.next, nextLabel: stage.nextIn, hoursLeft };
};
const rcEscalationNote = (age) => {
  const e = rcEscalate(age);
  if (!e.next) return 'Past the 3-day review window — needs your decision.';
  const d = e.hoursLeft >= 24 ? Math.round(e.hoursLeft / 24) + ' day' + (Math.round(e.hoursLeft / 24) === 1 ? '' : 's') : e.hoursLeft + ' hours';
  return `Escalates to ${e.nextLabel} in ${d} if still unmatched.`;
};

// Status is DERIVED from age, never hardcoded, so the state machine is the
// single source of truth.
const RC_EXCEPTIONS = [
  { id: 'EXC-118', dep: 'DEP-0617', source: 'Amazon',  reason: 'Top candidate is below the auto-match threshold.', amount: '$447.20', age: '4h', _conf: 71 },
  { id: 'EXC-117', dep: 'DEP-0620', source: 'Shopify', reason: 'Settlement window spans 2 payout cycles.', amount: '$1,120.00', age: '1d', _conf: 58 },
  { id: 'EXC-115', dep: 'DEP-0613', source: 'Square',  reason: 'No candidate orders within ±3 day window.', amount: '$312.40', age: '3d', _conf: 41 }
].map(e => ({ ...e, status: rcEscalate(e.age).status }));
  ...
  <div className="col gap-6">
    ...
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
      {[
        ...
        { lbl: 'Funds Settled', val: '$48.2K', note: 'Net · last 7 days', cls: 'up', accent: 'var(--accent)', prov: 'settled', amt: 48200 }
      ].map((m, i) => (
        <div key={i} className="metric metric-accent" style={{ '--cc-accent': m.accent }}>
          <div className="lbl">{m.lbl}{m.prov && <FigureInfo figure={m.prov} amount={m.amt} />}</div>
...
const RcOrderDepositLink = ({ o }) => {
  ...
  if (!ds) { note = 'Awaiting a matching payout.'; tone = 'warn'; }
...
// ---------- EXPENSE RECONCILIATION EXCEPTIONS ----------
// Framework: "Low-confidence classifications route to an exceptions queue. Only
// surface a clarifying question through Ask when confidence is low, never as a
// default step. The mission is the operator focuses on selling, not on
// answering Noodleflow's questions." So this is a queue the operator visits,
// not a prompt that interrupts them — and it only ever holds low-confidence rows.
const ATTRIBUTION_LABEL = {
  single_sku: 'Tied to one product',
  shared_allocation: 'Split across products',
  business_obligation: 'Business obligation'
};
const ALLOCATION_LABEL = {
  revenue_share: 'by revenue share',
  unit_volume: 'by unit volume',
  manual: 'by manual split'
};
const ExpenseExceptions = () => {
  const rows = window.expenseExceptions ? expenseExceptions() : [];
  const [asked, setAsked] = useState({});
  if (!rows.length) return null;
  return (
    <div className="card" style={{ padding: '18px 22px' }}>
      <SectionHead title="Expense classification exceptions"
        sub={`${rows.length} expenses Noodleflow could not classify confidently. Everything else was filed without asking you.`} />
      <div className="cost-exc" style={{ marginTop: 12 }}>
        {rows.map(e => (
          <div key={e.id} className="cost-exc-row">
            <div className="ce-main">
              <div className="ce-name">{e.name} · {fmt$(e.monthly)}/mo</div>
              <div className="ce-reason">
                Filed as {ATTRIBUTION_LABEL[e.product_attribution]}
                {e.allocation_method ? ' ' + ALLOCATION_LABEL[e.allocation_method] : ''}
                {' '}· seen in {e.source === 'both' ? 'banking and accounting' : e.source === 'banking_only' ? 'banking only' : 'accounting only'}
              </div>
            </div>
            <span className="cost-tag warn">Low confidence</span>
            {asked[e.id]
              ? <span className="rb-elig ok" style={{ margin: 0 }}>Sent to Ask</span>
              : <button className="btn btn-ghost btn-sm" onClick={() => { setAsked(a => ({ ...a, [e.id]: true })); NFToast.show('Clarifying question queued in Ask', { icon: 'ask', tone: 'accent' }); }}>Ask me about it</button>}
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="rc-detail rc-detail-panel rc-exc-panel">
        ...
        <div className="rc-money-strip rc-exc-money">
          ...
          <div className="rc-money-cell">
            ...
            <div className="rc-escal">{rcEscalationNote(e.age)}</div>
          </div>
        </div>
