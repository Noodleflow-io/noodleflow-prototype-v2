...
const ASK_ANSWERS = {
  default: {
    ...
    cites: ['Available', 'Reserves', 'Activity'], links: [['command-center', 'Command Center']]
  },
  spend: {
    lead: 'You can safely deploy about $20,000 this week.',
    body: 'Available is $44,200 — your connected balance of $166,800 minus $84,200 protected in reserves and $38,400 committed to scheduled payments. After this week’s obligations, roughly $20,000 is free without touching any reserve. Waiting for the Jun 24 deposit lifts that further.',
    cites: ['Available $44,200', 'Protected $84,200', 'Committed $38,400'], links: [['bk-accounts', 'Accounts'], ['bk-allocations', 'Reserves']]
  },
  partners: {
    ...
    cites: ['Payments', 'Vendors'], links: [['bk-payments', 'Payments'], ['bk-vendors', 'Vendors']]
  },
  inventory: {
    ...
    cites: ['Inventory Coverage 14d', 'Run · Inventory'], links: [['run', 'Run'], ['analyze', 'SKU-113']]
  },
  taxes: {
    ...
    cites: ['Tax Reserve', 'Reserves'], links: [['bk-allocations', 'Reserves']]
  }
};
const pickAnswer = (q) => {
        ...
        {messages.map((m, i) => m.role === 'user' ? (
          ...
          <div key={i} className="ask-msg-ai">
            ...
            <div className="content">
              ...
              {m.links && m.links.length > 0 && setRoute && (
                <div className="ask-cmd-links" style={{ marginTop: 10 }}>
                  {m.links.map((l, j) => <button key={j} className="ask-cmd-link" onClick={() => setRoute(l[0])}>→ {l[1]}</button>)}
                </div>
              )}
    ...
    <div className="col gap-6 fade-in">
      ...
      {tab === 'notifications' && (
        <div className="card" style={{ padding: '6px 24px 18px' }}>
          ...
          <SetRow title="Reduce motion &amp; animation" sub="Suppress non-essential animation across the product (accessibility)." right={<Toggle on={!motion} onClick={() => NFMotion.set(!motion)} />} />
        </div>
      )}
...
const Onboarding = ({ onClose, onComplete, onActivateBanking }) => {
  ...
  const footRight = step < 7
    ...
    : <>
        <button className="btn btn-ghost" onClick={onComplete}>Explore insights first</button>
