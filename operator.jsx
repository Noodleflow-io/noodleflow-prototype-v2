...
// Distinct jobs (same as Command Center comment):
//   Run      = today's action queue
//   Plan     = goal + model scenarios (evidence links back to source)
//   Grow     = revenue tools + channels
//   Insights = SKU performance / decision support
// Client feedback kept: freshness, sparklines, automation ledger on Run,
// Plan→source evidence deep-links, shared scenario engine, checkout Customize.
...
const NFFocus = {
  sku: null, cardId: null,
  set: (opts) => { NFFocus.sku = opts && opts.sku || null; NFFocus.cardId = opts && opts.cardId || null; },
  take: () => { const o = { sku: NFFocus.sku, cardId: NFFocus.cardId }; NFFocus.sku = null; NFFocus.cardId = null; return o; }
};
...
const WeeklyAutomationDigest = ({ total = '$7,140' }) => (
  <div className="card" style={{ padding: '18px 22px' }}>
    <div className="cc-panel-head" style={{ marginBottom: 10 }}>
      <h3 style={{ margin: 0, fontSize: 14, fontWeight: 600 }}>This week, NoodleFlow handled</h3>
      <Pill tone="ok" dot={false}>{total} impact</Pill>
    </div>
    <div className="digest-list">
      {WEEKLY_DIGEST.map((d, i) => (
...
const OperatorRun = ({ setRoute }) => {
  ...
  const stats = [
    { lbl: 'Cash Available', val: fmt$(cash.available), note: 'Synced from connected accounts', noteCls: 'up', accent: 'var(--accent)', prov: 'available', amt: cash.available },
    { lbl: 'Upcoming Expenses', val: fmt$(cash.committed), note: '3 items need attention', noteCls: 'warn', accent: 'var(--warn)', prov: 'committed', amt: cash.committed },
  ...
  return (
    <div className="col gap-6">
      <SectionHead title="Today's action queue" sub="Decisions that need you now. Overview health stays on Command Center." />
      <FreshnessLine note="2 actions waiting" />
      <section className="metric-strip">
        {stats.map((s, i) => (
          <div key={i} className="metric metric-accent" style={{ '--cc-accent': s.accent }}>
            <div className="lbl">{s.lbl}{s.prov && <FigureInfo figure={s.prov} amount={s.amt} />}</div>
      ...
      <SectionHead title="Recommendations" sub="Edit amounts, preview impact, then commit." />
      <div className="col gap-4">
        <InsightCard
          ...
          spark={{ points: [28, 26, 24, 22, 20, 18, 16, 14], color: 'var(--warn)', label: 'Days of coverage · 8 wk', delta: '14 days', dir: 'down', w: 220 }}
          ...
          model={{ title: 'Preview reorder size', unit: '%', presets: [{ label: '20%', factor: 20 }, { label: '50%', factor: 50 }, { label: '100%', factor: 100 }],
            ...scenarioPreview('reorder',
              (p) => { const e = skuUnitEcon(NF_SKUS[0]); return { skuIdx: 0, units: Math.round(600 * p / 100), cost: e.costPerUnit, price: e.pricePerUnit }; },
              (out) => { const cov = 14 + out.coverageGain; return [
                { l: 'Cash outlay', v: fmt$(out.outlay) },
                { l: 'Coverage', v: cov + 'd', d: cov >= 21 ? 'above threshold' : 'below', dir: cov >= 21 ? 'up' : 'down' },
                { l: 'Profit', v: fmt$(out.profitGain), dir: out.profitGain >= 0 ? 'up' : 'down' }
              ]; }) }}
          actions={[
        ...
        <InsightCard
          ...
          spark={{ points: [71, 73, 74, 76, 78, 80, 82, 84], color: 'var(--ok)', label: 'Operating cash · 90d', delta: '+18%', dir: 'up', w: 220 }}
...
const OperatorPlan = ({ setRoute }) => {
  ...
  const recs = [
    { title: 'Add a new product line', impact: 'Model upfront cost, bucket & tax impact', seed: { mode: 'product' },
      evidence: { label: 'Channel Performance', route: 'grow', cardId: 'grow-channels', why: 'Wholesale is up 21% MoM — headroom for a new line.' } },
    { title: 'Reorder SKU-113 (Ridgefield 5 oz)', impact: 'SKU-level coverage & contribution profit', seed: { mode: 'reorder', skuIdx: 0, units: 600 },
      evidence: { label: 'SKU-113 detail', route: 'analyze', sku: 'SKU-113', cardId: 'analyze-sku113', why: '32% of profit on 14% of volume, 19 days to stockout.' } },
    { title: 'Increase Ridgefield 5 oz price by 4%', impact: 'Est. impact: +$2,100/mo', seed: { mode: 'pricing', skuIdx: 0, pricePct: 4, adspend: 0 },
      evidence: { label: 'SKU-113 margin trend', route: 'analyze', sku: 'SKU-113', cardId: 'analyze-sku113', why: 'Margin rose 48% → 68% over 90 days with steady sell-through.' } }
  ];
  ...
  return (
    <div className="col gap-6">
      ...
      <FreshnessLine note="Recommendations recomputed hourly" />
      <div className="card" style={{ padding: '22px 24px' }}>
        <div className="plan-goal">
          ...
          <div className="pg-spark">
            <div className="pg-spark-wrap">
              <Sparkline points={[17200, 18400, 19100, 20300, 21200, 21700, 22100]} color="var(--accent)" w={300} h={46} />
              <div className="pg-spark-meta">
                <span>Monthly profit · last 7 months</span>
                <b>$22,100 <em>↑ 28%</em></b>
              </div>
            </div>
          </div>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {r.evidence && (
                <div className="pr-evidence">
                  <span className="pe-why">{r.evidence.why}</span>
                  <button type="button" className="pe-link" onClick={() => {
                    if (r.evidence.sku || r.evidence.cardId) NFFocus.set({ sku: r.evidence.sku, cardId: r.evidence.cardId });
                    setRoute(r.evidence.route);
                  }}>
                    <Icon name="arrow_right" size={11} /> {r.evidence.label}
                  </button>
                </div>
              )}
            </div>
...
const ChannelRow = ({ c, onClick }) => {
  ...
  return (
    <div className="chan-row row-clickable" onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{ position: 'relative' }}>
      ...
      <span className="chan-spark"><Sparkline points={c.trend} color={c.dir === 'up' ? 'var(--ok)' : 'var(--warn)'} w={62} h={24} /></span>
...
const OperatorGrow = ({ setRoute }) => {
  ...
  React.useEffect(() => {
    const f = NFFocus.take();
    if (f.cardId) {
      setFocusCard(f.cardId);
      setTimeout(() => { const el = document.getElementById(f.cardId); if (el) el.scrollIntoView({ behavior: 'smooth', block: 'center' }); }, 80);
      setTimeout(() => setFocusCard(null), 2200);
    }
  }, []);
  const channels = [
    { name: 'Shopify',   bar: 100, amt: '$88,400', delta: '+8%',  dir: 'up', orders: 1284, trend: [72,74,78,80,82,86,88] },
    { name: 'Amazon',    bar: 42,  amt: '$32,100', delta: '−3%',  dir: 'down', orders: 612, trend: [36,35,34,33,33,32,32] },
    { name: 'Wholesale', bar: 28,  amt: '$22,300', delta: '+21%', dir: 'up', orders: 47, trend: [14,16,18,19,20,21,22] }
  ];
  return (
    <div className="col gap-6">
      <SectionHead title="Revenue &amp; Growth" sub="Tools to generate more revenue, plus channel performance." />
      <FreshnessLine note="Channel data synced from connected stores" />
      <InsightCard
        id="grow-product-a" tone="danger" category="Products" status="Needs Action" confidence="moderate"
        statement="Revenue for Product A declined 18% over the past 30 days."
        spark={{ points: [17.2, 16.8, 16.1, 15.4, 14.9, 14.4, 14.1], color: 'var(--danger)', label: 'Revenue · last 30 days', delta: '−18%', dir: 'down', w: 240 }}
        ...
        automate={false} />
      <InsightCard
        id="grow-roas" className={focusCard === 'grow-roas' ? 'ic-focus' : ''} tone="ok" category="Marketing" status="Opportunity" confidence="high"
        statement="Paid social ROAS is running 1.4× above target on the Ridgefield line."
        spark={{ points: [2.6, 2.8, 3.1, 3.4, 3.6, 3.8, 3.9], color: 'var(--ok)', label: 'ROAS · last 7 weeks', delta: '3.9×', dir: 'up', w: 240 }}
        ...
        model={{ title: 'Preview budget increase', unit: '%', presets: [{ label: '+10%', factor: 10 }, { label: '+25%', factor: 25 }, { label: '+50%', factor: 50 }],
          ...scenarioPreview('pricing',
            (p) => { const e = skuUnitEcon(NF_SKUS[0]); return { skuIdx: 0, pricePct: 0, cost: e.costPerUnit, adspend: Math.round(budget * p / 100), roas: 3.9 }; },
            (out, inp) => [
              { l: 'Extra spend', v: fmt$(inp.adspend) },
              { l: 'Proj. revenue', v: fmt$(out.revenue), dir: 'up' },
              { l: 'Proj. profit', v: fmt$(out.profitGain), dir: out.profitGain >= 0 ? 'up' : 'down' }
            ]) }}
        actions={[
      ...
      <div className="grow-tools">
        {GROW_TOOLS.map(t => (
          <button key={t.key} type="button" className={`grow-tool ${tool === t.key ? 'active' : ''}`} onClick={() => setTool(t.key)}>
      ...
      <div id="grow-channels" className={`card${focusCard === 'grow-channels' ? ' ic-focus' : ''}`} style={{ padding: '8px 24px' }}>
        {channels.map((c, i) => <ChannelRow key={i} c={c} onClick={() => setChan(c)} />)}
      </div>

      {coOpen && <CheckoutBuilder tool={tool === 'campaign' ? 'link' : tool} onClose={() => setCoOpen(false)} />}
  ...
  return (
    <div className="skumap-row row-clickable" onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
      ...
      <div className="skumap-mid">
        <div className="sm-lbl" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <CostGuard sku={s.sku} fallback="Margin needs cost"><span>Margin {s.margin}%</span></CostGuard>
          <span className={`sku-tag ${s.tag}`}>{s.tagLabel}</span>
          <CostSourceTag sku={s.sku} compact />
        </div>
...
const OperatorAnalyze = ({ setRoute }) => {
  ...
  React.useEffect(() => {
    const f = NFFocus.take();
    if (f.sku) {
      const hit = NF_SKUS.find(s => s.sku === f.sku);
  ...
  return (
    <div className="col gap-6">
      ...
      <FreshnessLine note="Margin figures depend on per-SKU cost data" />
      <InsightCard
        id="analyze-sku113" className={focusCard === 'analyze-sku113' ? 'ic-focus' : ''} tone="ok" category="Analysis" status="Opportunity" confidence="high"
        ...
        spark={{ points: NF_SKUS[0].trend['90'], color: 'var(--ok)', label: 'Margin trend · last 90 days', delta: '48% → 68%', dir: 'up', w: 260 }}
        ...
        <div className="nf-toolbar">
          <div className="seg">
            {[['margin', 'Margin'], ['profit', 'Profit'], ['volume', 'Volume']].map(o => (
              <button key={o[0]} type="button" className={sort === o[0] ? 'on' : ''} onClick={() => setSort(o[0])}>{o[1]}</button>
            ))}
          </div>
          <div className="seg">
            {[['all', 'All'], ['winners', 'Winners'], ['risk', 'At risk']].map(o => (
              <button key={o[0]} type="button" className={filter === o[0] ? 'on' : ''} onClick={() => setFilter(o[0])}>{o[1]}</button>
            ))}
          </div>
        </div>
      } />
      <div className="card" style={{ padding: '8px 24px' }}>
        {skus.map((s) => <SkuRow key={s.sku} s={s} onClick={() => setDetail(s)} />)}
      </div>
      <CostExceptionsQueue onOpenSku={(s) => setDetail(s)} />
