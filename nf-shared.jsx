...
const NF_RESERVE_DEFAULTS = [
  { key:'payroll',   name:'Payroll Reserve',   amt:18800, color:'var(--accent)',   mode:'cycles', target:1.5,
    rule:'Maintain 1.5 payroll cycles in reserve.', detail:'Funded 12% of each deposit · next run Jun 30',
    history:[16000,16800,17400,18000,18800], project:'Target met' },
  { key:'obligations', name:'Obligations Reserve', amt:9600, color:'var(--danger)', mode:'fixed', target:9600,
    rule:'Hold rent, utilities, and recurring business obligations.', detail:'Funded ahead of due dates',
    history:[7200,7800,8400,9000,9600], project:'Next rent covered' },
...
const Modal = ({ title, sub, onClose, children, foot, width = 720 }) => {
  ...
  // Portal to <body>. A Modal is rendered inline beside its trigger, which may
  // sit inside a card carrying a transform (the hover lift on .metric). A
  // transformed ancestor becomes the containing block for position:fixed
  // descendants, which traps the scrim inside that card's box instead of the
  // viewport. Portaling keeps it viewport-relative wherever it was invoked.
...
const CashInfo = () => {
  ...
  return (
    <>
      <span className="nf-cash-info" title="How is this calculated?" onClick={(e) => { e.stopPropagation(); setOpen(true); }}>
        <svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">
          <circle cx="8" cy="8" r="6.3" fill="none" stroke="currentColor" strokeWidth="1.35" />
          <path d="M6.2 6.05a1.85 1.85 0 0 1 3.6.6c0 1.2-1.5 1.5-1.75 2.3" fill="none" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" />
...
// ============ NUMERIC FIELD ============
// Keeps the raw text while focused so clearing shows an empty field (not "0")
// and typing "2" doesn't become "02". Coerces to a number for the model.
const NumInput = ({ value, onChange, min = 0, className = 'co-input' }) => {
  const [txt, setTxt] = useState(String(value));
  const focused = useRef(false);
  useEffect(() => { if (!focused.current) setTxt(String(value)); }, [value]);
  return (
    <input className={className} type="text" inputMode="decimal" value={txt}
      onFocus={() => { focused.current = true; }}
      onBlur={() => {
        focused.current = false;
        const n = parseFloat(txt);
        const v = isNaN(n) ? min : Math.max(min, n);
        setTxt(String(v)); onChange(v);
      }}
      onChange={e => {
...
// ============ DATA PROVENANCE (feedback items 9, 10, 11) ============
// Two different things have been called "confidence":
//   · Match confidence  — the reconciliation engine's internal score. Stays
//     hidden from the merchant. Handled in reconcile.jsx, unchanged.
//   · Data provenance   — how sure a merchant can be that a figure is current,
//     complete and correctly sourced. This must be VISIBLE. That's this module.
// Attached to every dollar figure derived from connected-account data, so the
// interface never asserts more certainty than the data supports.
const NF_SYNC = {
  last: 'Today at 8:41 AM',
  method: 'Automatic · every 4 hours',
  accounts: [
    { name: 'Chase Business Checking', tail: '···4821', cleared: 118400, pending: 4200, synced: 'Today at 8:41 AM' },
    { name: 'Mercury Operating',       tail: '···9104', cleared: 42600,  pending: 1600, synced: 'Today at 8:39 AM' }
  ]
};

const NF_FIGURES = {
  available: { label: 'Available to deploy', estimate: false,
    note: 'Derived figure: total connected balance minus protected reserves minus committed payments.' },
  protected: { label: 'Protected in reserves', estimate: true,
    note: 'Includes merchant-set reserve targets, which are manual inputs rather than bank-derived values.' },
  committed: { label: 'Committed', estimate: false,
    note: 'Scheduled outbound payments in the next 14 days, read from your payment schedule.' },
  settled:   { label: 'Funds settled', estimate: false,
    note: 'Deposits matched to orders and cleared to your bank.' },
  reconcile: { label: 'Reconcile total', estimate: false,
    note: 'Sum of deposits ingested from connected payout sources.' },
  rule_commitments: { label: 'Committed to rules', estimate: true,
    note: 'A recurring monthly total, not a point-in-time balance. Percentage-based rules are projected against your monthly inflow, so this figure moves as revenue moves.' }
};

const ProvenanceBody = ({ figureKey = 'available', amount }) => {
  const f = NF_FIGURES[figureKey] || NF_FIGURES.available;
  const cleared = NF_SYNC.accounts.reduce((s, a) => s + a.cleared, 0);
  const pending = NF_SYNC.accounts.reduce((s, a) => s + a.pending, 0);
  return (
    <div className="prov">
      {amount != null && (
        <div className="prov-hero">
          <span className="ph-l">{f.label}</span>
          <span className="ph-v">{fmt$(amount)}</span>
        </div>
      )}
      <div className="prov-sec">Sourced from</div>
      {NF_SYNC.accounts.map((a, i) => (
        <div key={i} className="prov-acct">
          <div className="pa-top"><span className="pa-n">{a.name} {a.tail}</span><span className="pa-t">{a.synced}</span></div>
          <div className="pa-split">
            <span><i className="dot cleared" />Cleared {fmt$(a.cleared)}</span>
            <span><i className="dot pending" />Pending {fmt$(a.pending)}</span>
          </div>
        </div>
      ))}
      <div className="prov-rows">
        <div className="pr-row"><span>Last sync</span><b>{NF_SYNC.last}</b></div>
        <div className="pr-row"><span>Sync method</span><b>{NF_SYNC.method}</b></div>
        <div className="pr-row"><span>Cleared</span><b>{fmt$(cleared)}</b></div>
        <div className="pr-row"><span>Pending</span><b>{fmt$(pending)}</b></div>
        <div className="pr-row"><span>Manual override or estimate</span>
          <b className={f.estimate ? 'flag' : ''}>{f.estimate ? 'Yes — included' : 'None'}</b></div>
      </div>
      <div className="prov-note">{f.note}</div>
    </div>
  );
};

// Small "i" affordance placed beside any connected-data figure.
const Provenance = ({ figure = 'available', amount, label }) => {
  const [open, setOpen] = useState(false);
  const f = NF_FIGURES[figure] || NF_FIGURES.available;
  return (
    <>
      <span className="prov-trigger" title="Where does this number come from?"
        onClick={(e) => { e.stopPropagation(); setOpen(true); }}>
        <svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">
          <circle cx="8" cy="8" r="6.3" fill="none" stroke="currentColor" strokeWidth="1.35" />
          <circle cx="8" cy="5" r="0.9" fill="currentColor" />
          <path d="M8 7.5v4" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" />
        </svg>
      </span>
      {open && (
        <Modal title={label || f.label} sub={'Data provenance · synced ' + NF_SYNC.last} width={480}
          onClose={() => setOpen(false)}>
          <ProvenanceBody figureKey={figure} amount={amount} />
        </Modal>
      )}
    </>
  );
...
// ONE disclosure per figure. Two adjacent icons asked the reader to guess which
// question each answered — and they are really two halves of one question:
// how was this number built, and can I trust the inputs. Composition first
// (the arithmetic), then provenance (the source), in that order, because the
// equation is what a merchant reaches for and the source is what backs it up.
const FigureInfo = ({ figure = 'available', amount, label }) => {
  const cash = useCash();
  const [open, setOpen] = useState(false);
  const f = NF_FIGURES[figure] || NF_FIGURES.available;
  const amt = amount != null ? amount : cash[figure];
  return (
    <>
      <span className="prov-trigger" title="How this is calculated, and where it comes from"
        onClick={(e) => { e.stopPropagation(); setOpen(true); }}>
        <svg viewBox="0 0 16 16" width="14" height="14" aria-hidden="true">
          <circle cx="8" cy="8" r="6.3" fill="none" stroke="currentColor" strokeWidth="1.35" />
          <circle cx="8" cy="5" r="0.9" fill="currentColor" />
      ...
      {open && (
        <Modal title={label || f.label} sub={'Synced ' + NF_SYNC.last} width={520} onClose={() => setOpen(false)}>
          <div className="fig-info">
            <div className="prov-hero">
              <span className="ph-l">{label || f.label}</span>
              <span className="ph-v">{fmt$(amt)}</span>
            </div>

            {figure === 'available' && <CashEquation cash={cash} />}

            {figure === 'protected' && (
              <div className="nf-eq">
                <div className="nf-eq-title">What is being protected</div>
                {cash.reserves.map(r => (
                  <div key={r.key} className="nf-eq-row">
                    <span className="op" />
                    <span className="k"><span className="sw" style={{ background: r.color }} />{r.name}</span>
                    <span className="v">{fmt$(r.amt)}</span>
                  </div>
                ))}
                <div className="nf-eq-row total">
                  <span className="op">=</span><span className="k">Total protected</span>
                  <span className="v">{fmt$(cash.protected)}</span>
                </div>
              </div>
            )}
            ...
            {figure === 'committed' && (
              <div className="nf-eq">
                <div className="nf-eq-title">What is committed</div>
                <div className="nf-eq-row">
                  <span className="op" /><span className="k">Scheduled payments, next 14 days</span>
                  <span className="v">{fmt$(cash.committed)}</span>
                </div>
                <div className="nf-eq-row">
                  <span className="op" /><span className="k">Still available after these clear</span>
                  <span className="v">{fmt$(cash.available)}</span>
                </div>
              </div>
            )}

            <div className="fig-sep">Where this comes from</div>
            <ProvenanceBody figureKey={figure} />
          </div>
        </Modal>
      )}
    </>
...
// Freshness line — the timestamped device Command Center already uses, made
// reusable so Run / Plan / Grow / Insights can carry it too (feedback item 1).
const FreshnessLine = ({ note }) => (
  <div className="nf-fresh">
    <span className="fr-dot" />
    <span>Live data · synced {NF_SYNC.last}</span>
    {note && <span className="fr-note">· {note}</span>}
  </div>
);
...
Object.assign(window, {
  ...
  ConfidenceBadge, WhyDisclosure, InlineAmount, ModelThis, CashEquation, CashInfo, NumInput,
  NF_SYNC, NF_FIGURES, Provenance, ProvenanceBody, FreshnessLine, FigureInfo
});
